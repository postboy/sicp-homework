...
(define (invert-unit-series s)
  ; we use define instead of recursive call so that memo-proc optimization works
  (define result
    (cons-stream 1
		 (scale-stream (mul-series (stream-cdr s) result)
			       -1)))
  result)

(assert (stream-head (invert-unit-series integers) 5) '(1 -2 1 0 0))
