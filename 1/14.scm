...
Longest path depends on n lineary: as we can see, it has length (amount + kinds-of-coins). During the execution we should get down from (cc 11 5) to 1 first, and then get back accumulating results.

TODO:
Complexity: time -- at least O(n), space -- at least O(n).
